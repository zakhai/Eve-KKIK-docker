...
function start($n, $id, $t, $nets, $scripttimeout) {
	...
	switch ($n -> getNType()) {
		...
		case 'vpcs':
			...
			break;
		case 'dynamips':
			$cmd = 'rm -rf '.$n -> getRunningPath().'/netmap* 2>&1';
			exec($cmd, $o, $rc);
			error_log(date('M d H:i:s ').'INFO: Dynamips netmap deleted.');
			if ($rc !== 0) {
				error_log(date('M d H:i:s ').'ACTION: '.$cmd.'.');
				error_log(date('M d H:i:s ').'WARNING: '.implode("\n", $o));							
			}
			$cmd = '/opt/unetlab/wrappers/dynamips_wrapper -T '.$t.' -D '.$id.' -t "'.$n -> getName().'" -F /opt/unetlab/addons/dynamips/'.$n -> getImage().' -d '.$n -> getDelay();
			foreach ($n -> getSerials() as $interface_id => $interface) {
 				if ($interface -> getRemoteId() > 0) {					
					if($interface_id >= 128 && $interface_id < 144){
							$flags .= ' -p 0:16:WIC-2T -s 0:'.($interface_id - 112).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
					}

					if($interface_id >= 144 && $interface_id < 160){
							$flags .= ' -p 0:32:WIC-2T -s 0:'.($interface_id - 112).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
					}

					if($interface_id >= 16 && $interface_id < 32){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 1){
								$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();							
							}
						}
						
					}

					if($interface_id >= 32 && $interface_id < 48){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 2){
								$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
							}
					...
					if($interface_id >= 48 && $interface_id < 64){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 3){
							$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
							}
					...
					if($interface_id >= 64 && $interface_id < 80){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 4){
							$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
							}
					...
					if($interface_id >= 80 && $interface_id < 96){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 5){
							$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
							}
					...
					if($interface_id >= 96 && $interface_id < 112){
						foreach ($n -> getSlot() as $sid => $Module) {
							if($sid == 6){
							$flags .= ' -p '.$sid.':'.$Module.' -s '.$sid.':'.($interface_id - 16 * $sid).':unix:'.$n -> getRunningPath().'/netmap'.$interface_id.':'.$n -> getRunningPathA().''.$interface -> getRemoteId().'/netmap'.$interface -> getRemoteIf();
							}
