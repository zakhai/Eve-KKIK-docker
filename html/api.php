...
$app -> get('/api/status', function() use ($app, $db) {
	...
	$output['data']['yf_version'] = RVERSION;
	...
	list($output['data']['swap'],$output['data']['swaptotal']) = apiGetSwapUsage();
	$output['data']['memtotal'] = apiGetMemTotal();
	$output['data']['vCPU'] = apiGetvCPU();
	$output['data']['disktotal'] = apiGetDisktotal();
	list(
		$output['data']['iol'],
